...
function fetchCount($table_name, $column, $condition = 1)
{
    global $conn;
    $query = "SELECT COUNT($column) as `total` FROM `$table_name` WHERE $condition ";
    $result = mysqli_query($conn, $query);
    return mysqli_fetch_array($result)['total'];
}
...
function file_upload($image, $destination, $size_in_kb = "1024")
{
    ...
    $ext = pathinfo($image_size, PATHINFO_EXTENSION);
    if ($image['size'] > $image_size) {
        ...
        if ($ext != 'jpeg' || $ext != 'png' || $ext != 'jpg') {
            echo  warning_alert('Please Insert Image of type 
             than <b>' . $size_in_kb . ' Kb<b>');
        }
        reload();
    } else {
